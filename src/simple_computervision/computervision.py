...
def track_detect_video(video_filename: str,model_name: str = "yolov8n.pt",out_filename: str = "out_track.mp4",out_dir_name:str = "out_track", model_dir_name:str = "models",
                skip_frames: int = 1,method_type: str = "track"
                ):
    ...
    w,h,fps,fc = [ int(i) for i in [cap.get(cv2.CAP_PROP_FRAME_WIDTH),cap.get(cv2.CAP_PROP_FRAME_HEIGHT),cap.get(cv2.CAP_PROP_FPS),cap.get(cv2.CAP_PROP_FRAME_COUNT)]]
    ...
    while True:
        ...
        if (counter%skip_frames) == 0 :
            ...
            for c,id_,box in zip(classes,ids,boxes):
                ...
                cv2.putText(frame, put_str , (x1,y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
